...
void *clnt_connection(void *arg)
{
    ...
    char reg_line[BUFSIZ], reg_buf[BUFSIZ]; // HTTP 요청 메시지 읽어 저장
    ...
    /* 루트 경로(/) 요청에 대해 기본 페이지로 변경 */
    if(strcmp(filename, "HTTP/1.1") == 0 || strcmp(filename, "") == 0) {
    ...
    /* LED 제어 요청을 처리한다. */
    if(strcmp(filename, "led/on") == 0) {
        digitalWrite(LED, HIGH);
        printf("LED ON\n");
        sendOk(clnt_write);
        goto END;
    } else if(strcmp(filename, "led/off") == 0) {
        digitalWrite(LED, LOW);
        printf("LED OFF\n");
        sendOk(clnt_write);
        goto END;
...
int sendData(FILE* fp, char *ct, char *filename)
{
    ...
    fd = open(filename, O_RDONLY); 		/* 파일을 연다. */
    if(fd == -1) {
        return -1;
    }
